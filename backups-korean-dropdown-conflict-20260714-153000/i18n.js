(function () {
  ...
  const translations = {
    ...
    "RFQ Checklist": ["询价清单", "견적 요청 체크리스트"],
    ...
    "A practical RFQ should include material name, chemical formula, purity, shape, size, quantity, packaging preference, and target application.": ["有效的询价应包括材料名称、化学式、纯度、形状、尺寸、数量、包装要求和目标应用。", "효율적인 견적 요청에는 소재명, 화학식, 순도, 형상, 크기, 수량, 포장 방식 및 적용 분야가 포함되어야 합니다."],
    ...
    "Jiangxi Viilaa Metal Materials Co., Ltd is located in Nankang District, Ganzhou, Jiangxi. Use Google Maps for route planning and location reference.": ["江西中锡金属材料有限公司位于江西省赣州市南康区，可使用 Google 地图规划路线并查看位置。", "Jiangxi Viilaa Metal Materials Co., Ltd는 중국 장시성 간저우시 난캉구에 있습니다. Google 지도에서 위치와 경로를 확인할 수 있습니다."],
  ...
  function translatedText(source, language) {
    ...
    const item = translations[normalize(source)];
  ...
  function genericApplication(language, line) {
    ...
    const copy = {
      zh: {
        ...
        cvd: "适用于 CVD 前驱体、光学材料、催化及精密化学工艺。",
  ...
  function translateProductDetail(language) {
    ...
    if (infoPanel) {
      ...
      if (language === "zh") {
        ...
        featureParagraphs.forEach((item, index) => {
          item.hidden = index > 0;
          if (index === 0) item.textContent = genericFeature(language, productLine);
        });
        applicationParagraphs.forEach((item, index) => {
          item.hidden = index > 0;
          if (index === 0) item.textContent = genericApplication(language, productLine);
        });
      }
    }
  ...
  function createLanguageSwitcher(language) {
    ...
    switcher.addEventListener("pointerenter", () => window.clearTimeout(closeTimer));
